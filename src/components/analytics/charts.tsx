...
export const LegendDot: React.FC<{
  index: string | number;
  className?: string;
}> = React.memo(({ index, className }) => {
  const i =
    Math.abs(
      typeof index === "number"
        ? index
  ...
  return (
    <span
      aria-hidden
      className={`inline-block w-2.5 h-2.5 rounded-full ${bg} ${
        className || ""
      }`}
    />
  );
...
export const DonutChart: React.FC<DonutChartProps> = React.memo(
  ({ data, size = 240, thickness = 24, ariaLabel, centerLabel }) => {
...
export const CreationTrendChart: React.FC<CreationTrendChartProps> = React.memo(
  ({ data, ariaLabel }) => {
    const width = 600;
    const height = 160;
    const pad = 24;
    ...
    const pathD = points
      ...
      .join(" ");
    const areaD = `${pathD} L${pad + (data.length - 1) * stepX},${
      height - pad
    } L${pad},${height - pad} Z`;
    return (
      <svg
